...
从 `v1.6.1` 或更早版本升级到 AI 审核版，请直接更新到 `v1.7.2` 或更新的版本。`v1.7.0` 和 `v1.7.1` 删除了旧更新器仍会检查的 CF 脚本，可能在升级时失败并自动回滚。`v1.7.2` 保留一个仅提示功能已移除的兼容文件，不恢复 CF 验证。

更新成功会自动重启 Bot。若 `/ai` 仍提示未知指令，先运行 `sudo tg-bot version` 查看磁盘版本，再运行 `curl -fsS http://127.0.0.1:9000/healthz` 查看运行版本。若运行版本落后，执行 `sudo tg-bot restart`；若两者都还是旧版，需重新完成更新。升级后发送 `/start` 或点击工作台“刷新”获取新按钮。`/ai` 能打开但命令菜单未更新时，以运行 Bot 的普通用户在项目目录执行 `./venv/bin/python scripts/manage_webhook.py --commands-only`。
...
正式 Tag 部署会显示例如 `v1.6.1`；开发分支会显示例如 `v1.6.1+提交号`。
...
```bash
bash scripts/create-release.sh v1.6.1
```
...
```json
{"ok":true,"version":"1.6.1","db":"ok","broadcast_worker":"ok"}
```
