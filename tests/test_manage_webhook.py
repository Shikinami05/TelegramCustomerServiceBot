...
class CommandMenuTests(unittest.TestCase):
    ...
    def test_user_and_admin_command_scopes_are_separate(self, api_post) -> None:
        manage_webhook.configure_command_menus(
            "https://telegram.test",
            {20, 10},
            {10},
        )
        ...
        self.assertEqual(first_admin_payload["commands"], manage_webhook.OWNER_COMMANDS)
        self.assertEqual(second_admin_payload["commands"], manage_webhook.ADMIN_COMMANDS)

    def test_command_definitions_are_valid_and_unique(self) -> None:
        for commands in (
            manage_webhook.USER_COMMANDS,
            manage_webhook.ADMIN_COMMANDS,
            manage_webhook.OWNER_COMMANDS,
        ):
